...
def disable_auto_restart(script_key):
...
def load_data():
    ...
    try:
        ...
        sync_fn = globals().get('sync_user_files_from_disk')
        if callable(sync_fn):
...
def run_script_generic(script_path, script_owner_id, user_folder, file_name, message_obj,
    ...
    try:
        ...
        if is_py:
            check = subprocess.run(
                [
                    ...
                    'ast.parse(pathlib.Path(sys.argv[1]).read_text(encoding="utf-8", errors="ignore"))',
                    script_path
                ],
        ...
        auto_restart_enabled[script_key] = True
        restart_counts[script_key] = 0
...
def subscribe_command(message):
    ...
    try:
        target = int(parts[1])
        days = int(parts[2])
        if len(parts) > 3 and parts[3] in SUBSCRIPTION_PLANS:
            plan = parts[3]
        else:
            plan = next(iter(SUBSCRIPTION_PLANS), 'basic')
    except ValueError:
    ...
    plan_label = SUBSCRIPTION_PLANS.get(plan, {}).get('label', plan.title())
    send_animated_message(message.chat.id,
...
def stop_user_script(call, file_name):
    ...
    script_info = bot_scripts.get(script_key)
    if script_info:
        disable_auto_restart(script_key)
...
@bot.message_handler(commands=['r', 'run'])
@security_check
...
@bot.message_handler(commands=['s', 'stop'])
@security_check
...
@bot.message_handler(commands=['f', 'files'])
@security_check
def short_files_cmd(message):
    show_user_files(message)

@bot.message_handler(commands=['l', 'logs'])
@security_check
...
def restart_cmd(message):
    ...
    if len(parts) < 2:
        ...
        if not running:
            ...
            return
        markup = types.InlineKeyboardMarkup(row_width=1)
        for _, info in running:
            fname = info.get('file_name', '')
            markup.add(types.InlineKeyboardButton(
                f"🔄 {fname[:25]}", callback_data=f"restart_{fname}"))
        bot.reply_to(message, "🔄 Restart — Select script:", reply_markup=markup)
        return
    ...
    threading.Thread(
        target=run_script_generic,
        args=(sp, uid, user_folder, fname, message, stype),
        daemon=True).start()
...
def start_command_v3(message):
    ...
    try:
        ...
        if len(welcome) <= 1024:
            ...
            logger.debug(f"start photo skipped for long caption uid={uid} len={len(welcome)}")
    except Exception as _e:
