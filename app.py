...
TELEGRAM_MODULES = {
    ...
    # ✅ FIX: 'PIL' is the import name but 'Pillow' is the pip package name.
    # Users write `from PIL import Image` or `import PIL` — both map to Pillow.
        ...
        if file_ext == 'zip':
            ...
            try:
                with zipfile.ZipFile(tmp_path, 'r') as zf:
                    # ✅ SECURITY: Full path traversal + symlink protection
                    # Step 1: Pre-validate all member names before extracting anything
                    real_user_folder = os.path.realpath(os.path.abspath(user_folder))
                    for member in zf.namelist():
                        # Block absolute paths and directory traversal sequences
                        if member.startswith('/') or member.startswith('\\') or '..' in member.split('/'):
                            raise Exception(f"Unsafe ZIP member name: {member}")
                        # Check projected destination — catches encoded traversal
                        projected = os.path.realpath(os.path.abspath(
                            os.path.join(real_user_folder, member)))
                        if not projected.startswith(real_user_folder + os.sep) and projected != real_user_folder:
                            raise Exception(f"Path traversal blocked: {member}")
                    # Step 2: Extract files one-by-one (never extractall) to catch symlink attacks
                    extracted_paths = []
                    for member in zf.namelist():
                        target_path = os.path.realpath(os.path.abspath(
                            os.path.join(real_user_folder, member)))
                        zf.extract(member, real_user_folder)
                        extracted_paths.append(target_path)
                        # Step 3: Post-extract symlink check — OS may have created a symlink
                        if os.path.islink(target_path):
                            link_dest = os.path.realpath(target_path)
                            if not link_dest.startswith(real_user_folder + os.sep):
                                os.remove(target_path)
                                raise Exception(f"Symlink escape blocked: {member} → {link_dest}")
                    extracted = []
...
def start_command_v3(message):
    """Full onboarding — single clean message with everything a new user needs."""
    uid = message.from_user.id
    ...
    tier_label  = get_user_status_label(uid)
    file_limit  = get_user_file_limit(uid)
    file_count  = get_user_file_count(uid)
    ...
    welcome = (
        ...
        f"📦 Files: <b>{file_count}/{file_limit}</b>\n\n"

        f"<b>📌 What is this bot?</b>\n"
        f"Host and run your Python or Node.js bots 24/7 directly on Telegram. "
        f"Upload your script, hit Run — your bot starts instantly with auto-restart on crash.\n\n"

        f"<b>🚀 Quick Start (3 steps)</b>\n"
        f"1️⃣ Upload your <code>.py</code> or <code>.js</code> file\n"
        f"2️⃣ Tap <b>▶️ Run</b> to start it\n"
        f"3️⃣ Use <b>/files</b> to manage all your scripts\n\n"

        f"<b>📂 Supported File Types</b>\n"
        f"  ✅ <code>.py</code>  — Python 3 scripts\n"
        f"  ✅ <code>.js</code>  — Node.js scripts\n"
        f"  ✅ <code>.zip</code> — Extract multiple files at once\n"
        f"  ✅ <code>.env</code> — Environment variables for your script\n"
        f"  ❌ Other types — stored but cannot be executed\n\n"

        f"<b>⚡ Runtime Behaviour</b>\n"
        f"  • Scripts run <b>forever</b> — no time limit\n"
        f"  • If your script crashes → <b>auto-restarts immediately</b>\n"
        f"  • Use /stop to permanently stop a script\n"
        f"  • Use /restart to manually restart\n\n"

        f"<b>📋 All Commands</b>\n"
        f"  /files — View & manage your uploaded files\n"
        f"  /run — Run a script\n"
        f"  /stop — Stop a running script\n"
        f"  /restart — Restart a script\n"
        f"  /logs — View script output logs\n"
        f"  /livelog — Watch logs live in real time\n"
        f"  /upload — Upload a new file\n"
        f"  /delete — Delete a file\n"
        f"  /setenv — Set environment variables for a script\n"
        f"  /backup — Create a backup of your files\n"
        f"  /restore — Restore from a backup\n"
        f"  /stats — Your usage statistics\n"
        f"  /template — Use a pre-built bot template\n"
        f"  /help — Full help & command list\n\n"

        f"<b>⚠️ Limits</b>\n"
        f"  • Max file size: <b>{MAX_FILE_SIZE_MB} MB</b>\n"
        f"  • Max files stored: <b>{file_limit}</b>\n"
