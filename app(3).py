...
def run_script_generic(script_path, script_owner_id, user_folder, file_name, message_obj,
                       script_type='py', attempt=1):
    """Patched runner — injects env vars + sends completion/crash alerts."""
    script_key = f"{script_owner_id}_{file_name}"

    # Cooldown gate
    cd = check_cooldown(script_key)
    if cd > 0 and attempt == 1:
        try:
            bot.send_message(message_obj.chat.id,
                f"⏳ <b>Cooldown!</b> Wait {cd}s before restarting <code>{file_name}</code>.",
                parse_mode='HTML')
        except Exception:
            pass
        return
    ...
    if attempt > max_attempts:
        try:
            bot.send_message(message_obj.chat.id,
                f"❌ Failed after {max_attempts} attempts: <code>{file_name}</code>",
                parse_mode='HTML')
        except Exception:
            pass
        return
    ...
    try:
        if not os.path.exists(script_path):
            bot.send_message(message_obj.chat.id,
                f"❌ File not found: <code>{file_name}</code>", parse_mode='HTML')
            return
        ...
        if is_py:
            ...
            if chk.returncode != 0:
                bot.send_message(message_obj.chat.id,
                    f"⚠️ <b>Syntax Error:</b>\n<code>{chk.stderr[:400]}</code>",
                    parse_mode='HTML')
                debug_log(script_owner_id, f"SyntaxError in {file_name}: {chk.stderr[:200]}")
                return

        # Auto-scan on first run
        if attempt == 1:
            issues, _ = scan_script_for_issues(script_path)
            if issues:
                debug_log(script_owner_id, f"Issues in {file_name}: {'; '.join(issues)}")
        ...
        # Inject env vars (system + .env + per-script)
        env = inject_env_vars(script_key, script_owner_id, file_name)

        # Assign port if Flask/FastAPI detected
        try:
            with open(script_path, 'r', errors='ignore') as f:
                src_peek = f.read(3000)
            if any(x in src_peek for x in ['Flask','FastAPI','uvicorn','aiohttp']):
                assigned_port = get_next_port()
                port_registry[script_key] = assigned_port
                env['PORT'] = str(assigned_port)
                try:
                    bot.send_message(message_obj.chat.id,
                        f"📡 Web app detected! Assigned port <code>{assigned_port}</code>", parse_mode='HTML')
                except Exception:
                    pass
        except Exception:
            pass
        ...
        bot_scripts[script_key] = {
            'process': process, 'file_name': file_name, 'user_id': script_owner_id,
            'start_time': script_start_time, 'log_file': log_file,
            'log_path': log_file_path, 'script_key': script_key,
            'script_path': script_path, 'type': script_type
        }
        script_cooldown[script_key] = time.time()
        log_user_activity(script_owner_id, 'run')
        ...
        if process.poll() is None:
            success = (
                ...
                f"║ 📄 <b>File:</b> <code>{file_name[:25]}</code>\n"
                f"║ 🆔 <b>PID:</b> {process.pid}\n"
                f"║ ⏱️ <b>Started:</b> {script_start_time.strftime('%H:%M:%S')}\n"
                f"╚══════════════════════════════════════╝"
            )
            try:
                if msg:
                ...
                pass
            log_action(script_owner_id, "SCRIPT_START", f"{file_name} PID:{process.pid}")
            track_script_run()
            debug_log(script_owner_id, f"Script {file_name} started PID:{process.pid}")

            # Wait for completion in background thread
            def wait_for_completion():
                process.wait()
                exit_code = process.returncode
                runtime = (datetime.now() - script_start_time).total_seconds()
                log_file.close()
                # Read final log
                error_snippet = ""
                try:
                    ...
                    pass
                if exit_code == 0:
                    notify_script_completed(script_owner_id, file_name, runtime, exit_code)
                elif not auto_restart_enabled.get(script_key, False):
                    notify_crash_v2(script_key, file_name, script_owner_id, error_snippet)
                cleanup_script(script_key)
            ...
            if match:
                module = match.group(1).strip()
                installed = attempt_install_pip(module, message_obj) if is_py else False
                if installed:
                    cleanup_script(script_key)
                    run_script_generic(script_path, script_owner_id, user_folder,
                                       file_name, message_obj, script_type, attempt + 1)
                    return
            # Immediate crash
            notify_crash_v2(script_key, file_name, script_owner_id, error_output)
            try:
                err_preview = error_output[-600:]
                txt = (
                    f"❌ <b>Script failed immediately!</b>\n"
                    f"📄 <code>{file_name}</code>\n\n"
                    f"<code>{err_preview}</code>"
                )
                if msg:
                    bot.edit_message_text(txt, message_obj.chat.id, msg.message_id, parse_mode='HTML')
                else:
                    bot.send_message(message_obj.chat.id, txt, parse_mode='HTML')
            except Exception:
                pass
            cleanup_script(script_key)
    except Exception as e:
        logger.error(f"run_script_generic error: {e}")
        debug_log(script_owner_id, f"Runner error: {e}")
        try:
            bot.send_message(message_obj.chat.id, f"❌ Error starting script: {str(e)[:100]}")
        except Exception:
            pass


# ============================================================
# 🔔 IMPROVED SUBSCRIPTION EXPIRY ALERT (3 days + 1 day)
# ============================================================
...
def log_action(user_id, action, detail=""):
    _orig_log_action(user_id, action, detail)
    cmd = action.lower().replace('_', '')
    log_user_activity(user_id, cmd)
...
_orig_start = start_command.__wrapped__ if hasattr(start_command, '__wrapped__') else None
...
def process_custom_schedule(message, fname):
    ...
    if len(parts) < 3:
        bot.reply_to(message,
            "❌ Format: <code>YYYY-MM-DD HH:MM daily</code>\nExample: <code>2025-12-25 08:00 daily</code>",
            parse_mode='HTML')
        return
    date_str = parts[0]
    time_str = parts[1]
    repeat = parts[2].lower() if parts[2].lower() in ('once','daily','weekly','hourly') else 'once'
    try:
        run_at = datetime.strptime(f"{date_str} {time_str}", '%Y-%m-%d %H:%M')
    except ValueError:
        bot.reply_to(message, "❌ Invalid date/time format!\nUse: YYYY-MM-DD HH:MM")
        return
    try:
        conn = get_db()
        conn.execute(
            'INSERT INTO scheduled_jobs (user_id, file_name, run_at, repeat, active, created_at) VALUES (?,?,?,?,1,?)',
            (uid, fname, run_at.isoformat(), repeat, datetime.now().isoformat()))
        conn.commit(); conn.close()
    except Exception as e:
        bot.reply_to(message, f"❌ Error saving schedule: {str(e)[:80]}")
        return
    bot.reply_to(message,
        f"✅ <b>Custom Schedule Set!</b>\n"
        f"📄 <code>{fname}</code>\n"
        f"⏰ Run at: {run_at.strftime('%Y-%m-%d %H:%M')}\n"
        f"🔁 Repeat: {repeat}",
        parse_mode='HTML')


# ============================================================
# ℹ️ ENHANCED /help COMMAND
# ============================================================
    ...
    def wrapper(message, *args, **kwargs):
        ...
        if uid and maintenance_mode and uid not in admin_ids and uid != OWNER_ID:
            try:
                ...
                pass
            return
        # Admin PIN check for sensitive commands
        if uid and uid in admin_pin_required and not is_admin_pin_verified(uid):
            cmd = getattr(func, '__name__', '')
            sensitive = ['broadcast', 'ban', 'unban', 'grant', 'revoke', 'admin']
            if any(s in cmd for s in sensitive):
                msg = bot.reply_to(message, "🔐 Enter your Admin PIN to continue:")
                def verify_pin_step(m):
                    pin = (m.text or '').strip()
                    try:
                        bot.delete_message(m.chat.id, m.message_id)
                    except Exception:
                        pass
                    if verify_admin_pin(uid, pin):
                        bot.send_message(m.chat.id, "✅ PIN verified! Retry your command.")
                    else:
                        bot.send_message(m.chat.id, "❌ Wrong PIN!")
                bot.register_next_step_handler(msg, verify_pin_step)
                return
        return _orig_security_check_v2(func)(message, *args, **kwargs)
    return wrapper
